...
public class RestResponse {
    ...
    public ArrayList<?> verifyarrayCount() {
       return  response.body().jsonPath().get("$.");

    }

    public void verifySchema(String filePath) {
        response.then().assertThat().body(matchesJsonSchemaInClasspath(filePath));
    }

    public int getJpathvalue(String keyPath)
    {
       return response.body().jsonPath().get(keyPath);
    }

    public String getJpathvaluetext(String keyPath)
    {
        return response.body().jsonPath().get(keyPath);
